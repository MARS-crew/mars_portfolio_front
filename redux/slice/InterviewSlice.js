...
import {getInterview, getInterviewOne} from "../../api/v1/interview";
...
export const getInterviewOneAsync = createAsyncThunk(
    // string action type value: 이 값에 따라 pending, fulfilled, rejected가 붙은 액션 타입이 생성된다.
    'interview/get_one',
    // payloadCreator callback: 비동기 로직의 결과를 포함하고 있는 프로미스를 반환하는 비동기 함수
    async (payload,  thunkAPI) => {
        const source = axios.CancelToken.source();
        return getInterviewOne( payload.token, payload.id,{
            cancelToken: source.token,
        })
            .then(async function (response) {
                // console.log('interview resp');
...
export const InterviewSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(getInterviewOneAsync.fulfilled, (state, action) => {
            // Add user to the state array
            state.list = action.payload;
        })
...
export const getInterviewOneSelector = (state: RootState) => state.interviews.current;
