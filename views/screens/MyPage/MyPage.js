  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!extractedData.visitLog || extractedData.visitLog.includes('방문자가 없습니다')) {
          setNoLog(true);
        ...
        if (!extractedData.heart || extractedData.heart.includes('찜한 사용자가 없습니다.')) {
          setNoHert(true);
        ...
        if (!extractedData.Reviewlike || extractedData.Reviewlike.includes('리뷰에 좋아요한 사용자가 없습니다.')) {
          setNoReview(true);
